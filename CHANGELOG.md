...
- **Two more MCP tools, and deeper pacing.** `get_model_usage` answers "which model is burning the budget": per-model local Claude Code token totals over 1, 7, or 30 days across the consented homes, each model joined with its weekly meter utilization from the live snapshot (with the snapshot's own status and age alongside, so meter freshness is never implied; unmapped models stay visible with a null tier). `get_usage_history` reads the durable usage vault, which outlives Claude Code's ~30-day log cleanup: tokens per local calendar day with the sent/received split, window totals, and the top five projects by display name over 7, 30, or 90 days; days without a record are omitted, never reported as zero. `get_usage`'s per-tier `pace` now carries `cooldown_seconds` (idle time to fall back onto pace when ahead), `surplus_pct` (banked headroom when under), and `projected_final_pct` (where the meter lands at reset on current momentum, capped at 200). Both new tools sit behind the same per-home `mcp_roots` consent as the burn tool. The server's source-link allowlist widens to the vault reader for reads only; `TrustBoundaryTests` pins the list. (#56)
- The vault files are serialized through a compile-time System.Text.Json context (`VaultJsonContext`), which the trimmed MCP server needs and the widget gets for free.
- **Theme lint.** Every theme entry point (the Themes tab paste box, drop-in files, the MCP tool) now runs one validator: the schema (all fourteen `#rrggbb` colors, dial ranges) as errors, and the agent prompt's design rules as measured warnings (dark base by luminance, `text` at 4.5:1 or better on the composited card, a monochrome text ramp, a pace marker visible on every bar fill, one accent hue, the Mica opt-out). Findings name the field and the fix. A malformed hex in a drop-in file is now skipped with a reason instead of crashing the apply. The six built-ins lint clean.
- **Theme Studio.** Settings ▸ Themes ▸ "Studio" turns the paste box into a token editor: fourteen swatch-and-hex rows (each with the role's job as a tooltip), glass and border alpha sliders, a name, and the lint's findings updating as you type. The widget itself is the live preview: every valid edit re-tints it at once without touching the saved theme; Revert, Save, switching back to Paste, or closing Settings puts the saved theme back. "Start from" loads any built-in or installed theme; the dials the studio does not show (bloom, highlight, Matrix-style extras) ride along untouched. Save & apply writes `themes\<key>.json` and applies it; Copy JSON hands the draft to an agent for the next pass. The Paste box now shows the same findings before Save & apply.
- **`propose_theme` MCP tool.** Claude Code can hand the widget a palette: the server lints it first (a rejected palette comes back with findings and nothing written), then the widget lints again, saves it under `%APPDATA%\Sanduhr\themes\<key>.json`, applies it (or just saves with `apply: false`), shows "Theme 'X' applied from Claude Code" in the status line, and answers with the previous theme's key. The widget watches for the request file, so a proposal from the terminal lands within a second; requests older than ten minutes are ignored. A built-in's name is reserved. The server stays network-free and never writes a theme itself.
...
- **Claude Code statusline bridge.** Settings ▸ Claude Usage ▸ "Install statusline…" puts your usage percentages and reset times under the Claude Code prompt of the one home you pick, after a consent dialog and a timestamped backup of that home's `settings.json`; "Remove statusline" reverts the entry (only if it still points at Sanduhr's script) and deletes the snapshot. After each fetch the widget writes `%APPDATA%\Sanduhr\snapshot.json` atomically (percentages, reset times, plan, a short account hash; never keys or labels); on a failed fetch it keeps the last-good tiers so "stale" stays actionable.
- **`sanduhr-mcp`.** A read-only stdio MCP server (`get_usage`, `get_local_burn_by_project`, `ping`) over the same snapshot and your local Claude Code logs, built from `windows-dotnet/src/Sanduhr.Mcp`. It source-links four Core files and has no project or package references, so it cannot touch credentials or the network; `TrustBoundaryTests` pins that.
- **Publish usage (opt-in, off by default).** Nothing about your usage comes back to us. If you want it somewhere, send it to yourself: once a day Sanduhr can post token counts per project name (folder names only, never paths) to an endpoint you choose. Settings ▸ Publish usage has a Preset dropdown (Custom endpoint, or the 626 Labs dashboard preset, which fills the URL and auth for you and points you at your dashboard's Agents panel for the key), an Endpoint URL, an Auth scheme (bearer token, a header you name, or none), a masked "Set token" entry (Windows Credential Manager slot `publish:token`, never in a file), one "share" checkbox per detected Claude Code home (every home off until you tick it), a "Publish daily at" switch with the time (default 06:45 local), "Publish now", and a status line. Nothing is sent unless the switch is on, an endpoint is set, and at least one home is ticked. What goes out for a day: the date, your machine name, which homes you ticked, one token count per project name, a per-model-tier split, and the widget's current quota percentages (an explicit stale/no-data marker when they are not fresh, never stale numbers). Failed attempts retry no more than once per 15 minutes.
- **`publish_usage` MCP tool.** Queues the same upload through the widget (request/result files under `%APPDATA%\Sanduhr`); the MCP server itself holds no token and makes no network calls. Typed refusals `disabled` / `no_endpoint` / `no_token`, each with a remedy that names the setting, and `queued` when the widget is not running.

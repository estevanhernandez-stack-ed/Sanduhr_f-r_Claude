...
public class UsagePublisherTests
{
    ...
    [Fact]
    public void Payload_matches_the_manage_usage_record_contract()
    {
        var burn = new[]
        {
        ...
        var payload = UsagePublisher.BuildPayload(Day, "DESKTOP-X", burn, Roots(".claude-personal"), quota);
    ...
    private static JsonObject SamplePayload()
        => UsagePublisher.BuildPayload(Day, "m", new[] { Burn(".claude-personal", ("Sanduhr", "seven_day", 1)) },
            Roots(".claude-personal"), null);

    [Fact]
    public async Task Publish_200_is_ok_and_posts_bearer_json_to_the_endpoint()
    {
        var handler = new StubHandler(_ => Json(HttpStatusCode.OK, """{"ok":true}"""));

        var r = await UsagePublisher.PublishAsync(SamplePayload(), "sk-626-secret", handler);
        ...
        Assert.Equal(UsagePublisher.Endpoint, req.RequestUri!.AbsoluteUri);
    ...
    [Fact]
    public async Task Publish_401_is_a_key_error_that_never_echoes_the_key()
    {
        var handler = new StubHandler(_ => Json(HttpStatusCode.Unauthorized, """{"error":"unauthorized"}"""));
        var r = await UsagePublisher.PublishAsync(SamplePayload(), "sk-626-secret", handler);
        Assert.False(r.Ok);
        Assert.Equal(401, r.StatusCode);
        Assert.Contains("key", r.Message, StringComparison.OrdinalIgnoreCase);
    ...
    public async Task Publish_429_is_rate_limited()
    {
        ...
        var r = await UsagePublisher.PublishAsync(SamplePayload(), "k", handler);
        Assert.False(r.Ok);
    ...
    [Fact]
    public async Task Publish_without_a_key_never_hits_the_network()
    {
        var handler = new StubHandler(_ => Json(HttpStatusCode.OK, "{}"));
        var r = await UsagePublisher.PublishAsync(SamplePayload(), "  ", handler);
        Assert.False(r.Ok);

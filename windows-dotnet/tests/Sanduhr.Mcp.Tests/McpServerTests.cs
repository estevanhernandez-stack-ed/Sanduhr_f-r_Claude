...
public class McpServerTests
{
    ...
    public void Tools_list_has_three_read_only_tools_plus_publish_usage()
    {
        ...
        Assert.Contains("disabled / no_key", (string?)tools[3]!["description"]);
    }

    [Fact]
    public void Publish_tool_takes_only_a_strict_date_and_refuses_typed_over_the_wire()
    {
        using var tmp = new TempDir();
        var res = Run(tmp, Req(1, "tools/list"));
        var tools = (JsonArray)((JsonObject)res.Single()["result"]!)["tools"]!;
        ...
        // No settings path in the test config -> typed unavailable, isError false, no protocol error.
        var call = Run(tmp, Req(2, "tools/call", """{"name":"publish_usage","arguments":{"date":"2026-07-25"}}"""));
        var result = (JsonObject)call.Single()["result"]!;
        ...
        Assert.Equal("unavailable", (string?)payload["reason"]);

        // A non-string date is typed invalid_params, not a crash.
        var bad = Run(tmp, Req(3, "tools/call", """{"name":"publish_usage","arguments":{"date":7}}"""));
        var badPayload = (JsonObject)JsonNode.Parse((string)((JsonObject)bad.Single()["result"]!)["content"]![0]!["text"]!.GetValue<string>())!;
        Assert.Equal("invalid_params", (string?)badPayload["reason"]);
    }
